...
setup(void)
{
	...
	if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
		...
		if (centered) {
			mw = MAX(MAX(max_textw() + promptw, min_width), info[i].width);
...
read_Xresources(void) {
	...
	if ((xrm = XResourceManagerString(drw->dpy))) {
		...
		if (XrmGetResource(xdb, "dmenu.color4", "*", &type, &xval) == True)  /* normal background color */
			colors[SchemeNorm][ColBg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.color0", "*", &type, &xval) == True)  /* normal foreground color */
